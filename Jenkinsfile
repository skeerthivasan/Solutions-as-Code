  ...
  def build_solution(sol) {
	...
	dir(path) {
	    ...
	    println "Installing and conifguring the solution"
            ...
            if  (solname == 'Oracle') {
                sh script: "cd /root/production_runs/oracle-ansible;export ANSIBLE_COLLECTIONS_PATHS=/root/.ansible/collections;export ANSIBLE_ROLES_PATH=/root/.ansible/collections/ansible_collections/opitzconsulting/ansible_oracle/roles;export ANSIBLE_PYTHON_INTERPRETER=/usr/bin/python3.6;ansible-playbook -i inventory-asm  -e hostgroup=dbfs  playbooks/single-instance-asm.yml --private-key "  + '${SSH_KEY}' + " --user ansible  -v"
            }
            if  (solname == 'veeam') {
		dir("/var/lib/jenkins/workspace/Solution-automation/modules/veeam-setup") {
                  def vpath = workspace + "/" + "modules" + "/" + "veeam-setup".trim()
